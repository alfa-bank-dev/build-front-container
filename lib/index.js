...
function buildContainer(outputDir, args) {
    ...
    console.log('===== Start build docker =====');
    shell.exec('cd ' + outputDir + ' && docker build -t ' + args['docker-registry'] + '/' + args['container-name'] + ':' + version + ' .');
    console.log(`===== Start push docker to ${args['docker-registry']}/${args['container-name']}:${version}  =====`);
    shell.exec('cd ' + outputDir + ' && docker push ' + args['docker-registry'] + '/' + args['container-name'] + ':' + version);
}
...
function buildProject(buildOnHost, dockerRegistry, buildContainerVersion) {
    ...
    shell.exec('docker pull ' + dockerRegistry + '/' + NAME + ':' + buildContainerVersion);
    shell.exec('docker run -d -v $(pwd):/app-source --name=' +
        containerName + ' --net=host ' + dockerRegistry + '/' + NAME + ':' + buildContainerVersion);
    shell.exec('docker wait ' + containerName);
    shell.exec('docker logs ' + containerName);
    shell.exec('docker rm -f ' + containerName + ' || true');
}
